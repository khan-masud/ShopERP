...
const envSchema = z.object({
  ...
  DB_POOL_LIMIT: z.coerce.number().int().min(1).max(100).default(5),
  DB_POOL_QUEUE_LIMIT: z.coerce.number().int().min(1).max(5000).default(200),
  DB_CONNECT_TIMEOUT_MS: z.coerce.number().int().min(1000).max(60000).default(10000),
  DB_RETRY_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(5).default(3),
  DB_RETRY_BASE_DELAY_MS: z.coerce.number().int().min(10).max(5000).default(120),
